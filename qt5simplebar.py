...
class MainWin(QtWidgets.QMainWindow):
    ...
    def eventFilter(self, object, event):
        if event.type() == QtCore.QEvent.Enter:
            todate = QtCore.QDate().currentDate()
            to_day = todate.dayOfWeek()
            to_day_name = QtCore.QDate.longDayName(to_day)
            self.tlabel.setToolTip(to_day_name+" "+str(to_day))
            return True
        return False
...
class closeWin(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        #
        canc_icon = QtGui.QIcon("icons/cancel.svg")
        self.cancBtn = QtWidgets.QPushButton(canc_icon, "Cancel")
        self.cancBtn.clicked.connect(self.on_cancel)
        self.mbox.addWidget(self.cancBtn)
        #
        if self.window.cw_is_shown:
            self.window.cw_is_shown.close()
            self.window.cw_is_shown = None
