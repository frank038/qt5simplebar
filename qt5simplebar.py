...
class MainWin(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.tlabel_tooltip = ""
        self.tlabel.setToolTip(self.tlabel_tooltip)
        self.tlabel.setToolTipDuration(3000)
...
class menuWin(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.window = window
        self.setWindowFlags(window.windowFlags() | QtCore.Qt.FramelessWindowHint | QtCore.Qt.WindowStaysOnTopHint | QtCore.Qt.Tool)
        ####### box principale per il frame
        ...
        if with_compositor:
            ...
            self.frame.setStyleSheet("background: palette(window); border-radius:15px")
    ...
    def on_btn_clicked(self):
        ...
        for el in cat_list:
            ...
            if file_info.exists():
                ...
                #
        try:
            self.listWidget.customContextMenuRequested.disconnect(self.listItemRightClickedToRemove)
        except:
            pass
    ...
    def on_pref_clicked(self):
        ...
        for el in prog_list:
            ...
            if file_info.exists():
                ...
                #
        try:
            self.listWidget.customContextMenuRequested.disconnect(self.listItemRightClicked)
        except:
            pass
        self.listWidget.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.listWidget.customContextMenuRequested.connect(self.listItemRightClickedToRemove)
        self.listWidget.sortItems(QtCore.Qt.AscendingOrder)
...
class calendarWin(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.ldatebox.setContentsMargins(0,0,10,0)
...
"}")
        ...
        l_e = []
        # lista degli eventi: self.list_events
        for ev in self.window.list_events:
            if int(ev.DTSTART[4:6]) == int('{:02d}'.format(thisMonth)):
                yy = int(ev.DTSTART[0:4])
                mm = int(ev.DTSTART[4:6])
                dd = int(ev.DTSTART[6:8])
                l_e.append((QtCore.QDate(yy, mm, dd), ev.DESCRIPTION))
        
        c_dict = dict(l_e)
        ###
        self.calendar = Calendar(self, c_dict, self.vbox)
...
class Calendar(QtWidgets.QCalendarWidget):
    ...
    def popCalEv(self, date):
        ...
        for k,e in self.events.items():
            if k == date:
                label = ClickLabel()
                label.setText(appointment_char+" "+e)
                ...
                label.clicked.connect(self.event_clicked)
    ...
    def event_clicked(self):
        ...
        # remove all the existent widgets
        for i in reversed(range(self.cvbox.count())): 
            if self.cvbox.itemAt(i).widget() == label:
                self.cvbox.itemAt(i).widget().deleteLater()
    
    # #
    # def event_selected(self, event):
        # if event.button() == QtCore.Qt.LeftButton:
            # if event.type() == QtCore.QEvent.MouseButtonDblClick:
                # print("left double click", self.sender())

    
    def eventFilter(self, source, event):
        if (event.type() == QtCore.QEvent.MouseButtonDblClick) and (source is self.vw.viewport()):
            index = self.vw.indexAt(event.pos())
            return super(Calendar, self).eventFilter(source, event)
        else:
            return False
...
class closeWin(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.mbox.setContentsMargins(10, 10, 10, 10)
        self.frame = QtWidgets.QFrame(self)
    ...
    #
    def on_shut(self):
        print("on_shut:", shutdown_command)
        self.close()
    
    #
    def on_rest(self):
        print("on_rest:", restart_command)
        self.close()
    
    #
    def on_logo(self):
        print("on_logo:", logout_command)
        self.close()
...
if __name__ == '__main__':
    ...
    _window.set_wm_hints(flags=(Xutil.InputHint|Xutil.StateHint),
    ...
    # check for change in the application directories
